...
import testCases
from dnn_units import sigmoid,sigmoid_backward,relu,relu_backward
import lr_utils
...
def initialize_parameters_deep(layers_dims):
    ...
    for i in range(1,len(layers_dims)):
        Wi='W'+str(i)
        bi='b'+str(i)
        parameters[Wi]=np.random.randn(layers_dims[i],layers_dims[i-1])/ np.sqrt(layers_dims[i - 1])
        parameters[bi]=np.zeros((layers_dims[i],1))
        assert (parameters[Wi].shape==(layers_dims[i],layers_dims[i-1]))
        assert (parameters[bi].shape==(layers_dims[i],1))
    return parameters
    # np.random.seed(3)
    # parameters = {}
    # L = len(layers_dims)
